...
describe('group test', () => {
  ...
  test('verify group replaceMember', async () => {
    debug('Into verify group replaceMember ...')
    trs = lib.AschJS.transaction.createMultiSigTransaction({
      type: 504,
      fee: 100000000,
      senderId: groupAddress,
      args: [
        group.members[4].address,
        group.newMembers[0].address,
        1,
        group.m,
      ],
    ...
    debug(`--->replace group member '${group.members[4].address}' with '${group.newMembers[0].address}'`)
    await lib.submitTransactionAsync(trs)
    await lib.onNewBlockAsync()

    // TODO: verify record was replaced in table group_members
  })
  ...
  test('verify negative group replaceMember', async () => {
    ...
    trs = lib.AschJS.transaction.createMultiSigTransaction({
      type: 504,
      fee: 100000000,
      senderId: group.newMembers[1].address,
      args: [
        group.members[4].address,
        group.newMembers[0].address,
        1,
